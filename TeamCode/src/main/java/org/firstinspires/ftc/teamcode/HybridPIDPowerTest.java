...
public class HybridPIDPowerTest extends OpMode {
    ...
    private static final double MAX_TICKS_PER_SECOND = 2500.0; // Approximate max velocity in ticks/sec
    ...
    public void loop() {
        ...
        // Right Bumper: Increase shooter speed by 0.1
        if (gamepad1.right_bumper && !lastGamepad1RightBumperState) {
            shooterSpeed = Range.clip(shooterSpeed + 0.1, 0.0, 1.0);
        }
        lastGamepad1RightBumperState = gamepad1.right_bumper;

        // Left Bumper: Decrease shooter speed by 0.1
        ...
        double velocityPercent = velocity / MAX_TICKS_PER_SECOND;

        if (Math.abs(velocityPercent - shooterSpeed) <= 0.05 && shooterSpeed > 0 && !isShooting && !transfersOpen) {
            transfersOpen = true;
            leftTransfer.setPosition(0.5);
            rightTransfer.setPosition(0.0);
            shootTimer.reset();
        ...
        if (isShooting && shootTimer.seconds() >= 2.5) {
            transfersOpen = false;
            leftTransfer.setPosition(0.0);
            rightTransfer.setPosition(0.5);
            isShooting = false;
        ...
        // ========== TELEMETRY ==========
        telemetry.addData("Target Speed", "%.2f", shooterSpeed);
        telemetry.addData("Actual Speed", "%.0f ticks/s (%.2f%%)", velocity, velocityPercent);
